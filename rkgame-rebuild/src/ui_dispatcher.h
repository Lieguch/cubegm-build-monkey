...
/* UI 页面状态（与 gl_* 布尔值对应；P1-2 加 RECENT/SHOUCAANG 共 8 页） */
typedef enum {
    UIP_MAIN = 0,
    UIP_LIST,
    UIP_SEARCH,
    ...
    UIP_SETTING,
    UIP_BROWSER,
    UIP_RECENT,        /* P1-2: 最近页（复用 type.raw 背景） */
    UIP_SHOUCAANG,     /* P1-2: 收藏页（复用 search.raw 背景） */
    UIP_COUNT
} ui_page_state_t;
...
/* 获取当前页面状态（从 gl_* 布尔值推断；P1-2 加 recent/shoucang） */
