...
/* P1-3: 九桶浏览（TYPE 页 A 确认进桶）
 * bucket = 0-8，对应 dir "000".."008"。线性过滤，g_gl.count 控制循环
 * （未加载时 count=0 自动返回 0 / -1，安全）。 */
int game_list_dir_count(int bucket)
{
    if (bucket < 0 || bucket > 8) return 0;
    if (g_gl.count <= 0 || !g_gl.entries) return 0;
    char want[16];
    snprintf(want, sizeof(want), "%03d", bucket);
    int n = 0;
    for (int i = 0; i < g_gl.count; i++) {
        if (strcmp(g_gl.entries[i].dir, want) == 0) n++;
    }
    return n;
}

/* 返回桶内第 local 个条目的**全局索引**（-1 表示越界/空桶） */
int game_list_dir_index(int bucket, int local)
{
    if (bucket < 0 || bucket > 8 || local < 0) return -1;
    if (g_gl.count <= 0 || !g_gl.entries) return -1;
    char want[16];
    snprintf(want, sizeof(want), "%03d", bucket);
    int seen = 0;
    for (int i = 0; i < g_gl.count; i++) {
        if (strcmp(g_gl.entries[i].dir, want) == 0) {
            if (seen == local) return i;
            seen++;
        }
    }
    return -1;
