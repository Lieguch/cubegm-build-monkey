...
/* ============================================================
 * Save State（快照）— P0-B 用户终极目标
 * ============================================================
 * libretro API：retro_serialize_size / retro_serialize / retro_unserialize
 * 存档路径：states/<core>/<rom>.state.<slot>
 * 热键：<savestatehotkey>（默认 -1 = 禁用）
 */
                              ...
                              int slot, char *out, size_t out_size)
{
    char file_base[256] = "";
    const char *slash = strrchr(rom_path, '/');
    char *name = slash ? slash + 1 : (char *)rom_path;
    strncpy(file_base, name, sizeof(file_base) - 1);
    char *dot = strrchr(file_base, '.');
    if (dot) *dot = '\0';
    snprintf(out, out_size, "%sstates/%s/%s.state.%d",
             work_path, core_name, file_base, slot);
}

static void sstate_get_core_and_rom(char *core_name, size_t cn_size,
                                    char *rom_path, size_t rp_size)
{
    core_name[0] = '\0';
    rom_path[0] = '\0';
    if (sram_state.path[0]) {
        const char *saves = strstr(sram_state.path, "saves/");
        if (saves) {
            const char *cs = saves + 6;
            const char *slash2 = strchr(cs, '/');
            if (slash2) {
                size_t cl = slash2 - cs;
                if (cl < cn_size) { memcpy(core_name, cs, cl); core_name[cl] = '\0'; }
            }
        }
        const char *basename = strrchr(sram_state.path, '/');
        if (basename) {
            strncpy(rom_path, basename + 1, rp_size - 1);
            char *dot = strrchr(rom_path, '.');
            if (dot) strcpy(dot, ".zip");
        }
    }
    if (!core_name[0]) strcpy(core_name, "misc");
    if (!rom_path[0]) strcpy(rom_path, "misc/game.zip");
}

int sstate_save(int slot)
{
    if (!ctx.retro_serialize_size || !ctx.retro_serialize) {
        ERR("sstate_save: core missing retro_serialize"); return -1;
    }
    size_t size = ctx.retro_serialize_size();
    if (size == 0 || size > SSTATE_MAX_SIZE) {
        ERR("sstate_save: invalid size %zu", size); return -1;
    }
    uint8_t *buf = malloc(size);
    if (!buf) return -1;
    bool ok = ctx.retro_serialize(buf, size);
    if (!ok) { ERR("sstate_save: serialize failed"); free(buf); return -1; }

    char path[600], core_name[128], rom_path[600];
    sstate_get_core_and_rom(core_name, sizeof(core_name),
                            rom_path, sizeof(rom_path));
    sstate_build_path(rom_path, core_name, slot, path, sizeof(path));
    ...
    if (!fp) { ERR("sstate_save: cannot open %s", path); free(buf); return -1; }
    size_t n = fwrite(buf, 1, size, fp);
    int fd = fileno(fp);
    if (fd >= 0) fsync(fd);
    fclose(fp); free(buf);
    if (n != size) { ERR("sstate_save: short write"); return -1; }
    g_sstate_slot = slot;
    LOG("sstate_save: slot %d -> %s (%zu B)", slot, path, size);
    return 0;
}

int sstate_load(int slot)
{
    if (!ctx.retro_unserialize) {
        ERR("sstate_load: core missing retro_unserialize"); return -1;
    }
    char path[600], core_name[128], rom_path[600];
    sstate_get_core_and_rom(core_name, sizeof(core_name),
                            rom_path, sizeof(rom_path));
    ...
    if (!fp) { LOG("sstate_load: no file %s", path); return -1; }
    fseek(fp, 0, SEEK_END);
    long fsize = ftell(fp);
    fseek(fp, 0, SEEK_SET);
    if (fsize <= 0 || fsize > SSTATE_MAX_SIZE) { fclose(fp); return -1; }

    uint8_t *buf = malloc((size_t)fsize);
    if (!buf) { fclose(fp); return -1; }
    size_t n = fread(buf, 1, (size_t)fsize, fp);
    fclose(fp);
    if (n != (size_t)fsize) { free(buf); return -1; }

    bool ok = ctx.retro_unserialize(buf, (size_t)fsize);
    free(buf);
    if (!ok) { ERR("sstate_load: unserialize failed"); return -1; }
    g_sstate_slot = slot;
    LOG("sstate_load: slot %d <- %s (%ld B)", slot, path, fsize);
    return 0;
}

int sstate_check_hotkey(void)
{
    if (g_cfg.savestatehotkey < 0) return 0;
    if (!core_handle) return 0;

    /* 修复：不再 read() evdev fd（避免消费 joy_poll 的事件流）。
     * 改为读取 joy_poll() 已累积的 joy_key_state[] 全局 bitmask。
     * joy_key_state 由 joy_update_state() 在每次 joy_poll() 时更新。 */
    /* P1-1: 修复热键检测。
     * 设备 setting.xml 存的是**原厂事件码 bitmask**（如 3072=0xC00=TL1|TR1），
     * 不是本地 KEY_* 位索引。旧代码 1u<<hotkey 对 3072 直接溢出，
     * 且对 gamemenuhotkey=9（SELECT|START）也会误读。
     * 修复：把本地 joy state 翻成 factory keycode 后做 AND 全包含检测。 */
    static uint32_t last_factory_state = 0;
    uint32_t cur_factory = keymask_to_keycode(joy_all_keys_state());
    uint32_t target = (uint32_t)g_cfg.savestatehotkey;
    if (target &&
        ((cur_factory & target) == target) &&
        !(last_factory_state & target) /* 至少一个目标键新按下 */)
    {
        if (sstate_save(g_sstate_slot) == 0)
            g_sstate_slot = (g_sstate_slot + 1) % SSTATE_MAX_SLOTS;
        last_factory_state = cur_factory;
        return 1;
    }
    last_factory_state = cur_factory;
    return 0;
...
void sstate_set_slot(int slot) { g_sstate_slot = slot; }
int sstate_get_slot(void) { return g_sstate_slot; }

/* 检查指定 slot 是否有存档文件 */
int sstate_has_slot(int slot)
{
    if (slot < 0 || slot >= SSTATE_MAX_SLOTS) return 0;
    char path[512];
    build_sstate_path(path, sizeof(path), slot);
    struct stat st;
