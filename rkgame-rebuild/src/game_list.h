...
/* P1-3: 九桶浏览（TYPE 页 A 确认进桶 000-008）
 * 线性按 entries[].dir == "NNN" 过滤，不动全局状态。 */
int  game_list_dir_count(int bucket);   /* bucket 0-8 → 桶内条目数 */
int  game_list_dir_index(int bucket, int local); /* 桶内第 local 个 → 全局索引，-1 越界 */
