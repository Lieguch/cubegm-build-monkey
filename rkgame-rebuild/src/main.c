...
/* P1-3: 启动指定全局索引的游戏（列表态 A 键 与 TYPE 桶内 A 键共用）。
 * 返回 0 = 成功（已调 core_load），-1 = 越界未启动。
 * 调用方在返回 0 后应 return（核心已接管）。 */
static int menu_launch_global(int global_idx)
{
    const game_entry_t *ge = game_list_get(global_idx);
    if (!ge) {
        LOG("menu_launch_global: index %d out of range, skip", global_idx);
        return -1;
    }
    LOG("menu_launch_global: launching %d: %s (%s)",
        global_idx, ge->path, ge->core);
    ...
    LOG("menu_launch_global: %s with core %s",
...
static void main_menu(void)
{
    ...
    /* P1-2: 六页切页环新增两态（recent/shoucang），复用 type/search 背景 */
    bool  gl_recent      = false;          /* 最近页（mui_recent） */
    bool  gl_shoucang    = false;          /* 收藏页（mui_shoucang） */
    const int GL_PAGE_SIZE = 8;            /* 每页显示的游戏数 */

    /* P1-2: 六页切页环（对齐原厂 DAT_003af26c 页码 0-5）
     *   0=menu/list, 1=type, 2=recent, 3=shoucang, 4=search, 5=setting
     *   L1(factory TL1=0x400)=上页, R1(factory TR1=0x800)=下页，环回。
     *   gl_browser 为本地扩展，不进入环。 */
    int ring_cur = 0;
    #define RING_APPLY() do { \
        gl_showing   = (ring_cur == 0); \
        gl_typing    = (ring_cur == 1); \
        gl_recent    = (ring_cur == 2); \
        gl_shoucang  = (ring_cur == 3); \
        gl_searching = (ring_cur == 4); \
        gl_setting   = (ring_cur == 5); \
        gl_browser   = false; \
        /* P1-3: 模块 open/close 同步（进入对应环页才 open） */ \
        if (ring_cur == 1) { if (!ui_type_is_open()) ui_type_open(); } \
        else                { if (ui_type_is_open())  ui_type_close(); } \
        if (ring_cur == 2) { if (!ui_recent_is_open()) ui_recent_open(); } \
        else                { if (ui_recent_is_open())  ui_recent_close(); } \
        if (ring_cur == 3) { if (!ui_shoucang_is_open()) ui_shoucang_open(); } \
        else                { if (ui_shoucang_is_open())  ui_shoucang_close(); } \
    } while (0)
    ...
    #define MENU_TYPE     (1u << 8)   /* L1 = KEY_L1 = P1-2 环「上一页」(factory 0x400) */
    #define MENU_R1       (1u << 9)   /* R1 = KEY_R1 = P1-2 环「下一页」(factory 0x800) */
    #define MENU_BROWSER  (1u << 10)  /* L2 = KEY_L2 = 文件浏览器（本地扩展，独立于环） */
    ...
    while (1) {
        ...
        /* P1-2: L1 = factory TL1 (0x400) = 六页环「上一页」
         * 旧行为（误指 setting 单页开关）已废弃。 */
        if (pressed & MENU_TYPE) {
            ring_cur = (ring_cur + 5) % 6;   /* prev page */
            RING_APPLY();
            gl_showing = false;              /* 离开列表态，落到对应环页 */
            LOG("ring: L1 prev page -> %d (%s)",
                ring_cur,
                ring_cur==0?"menu/list":ring_cur==1?"type":ring_cur==2?"recent":
                ring_cur==3?"shoucang":ring_cur==4?"search":"setting");
        }

        /* P1-2: R1 = factory TR1 (0x800) = 六页环「下一页」（环回 menu） */
        if (pressed & MENU_R1) {
            ring_cur = (ring_cur + 1) % 6;   /* next page */
            RING_APPLY();
            gl_showing = false;
            LOG("ring: R1 next page -> %d (%s)",
                ring_cur,
                ring_cur==0?"menu/list":ring_cur==1?"type":ring_cur==2?"recent":
                ring_cur==3?"shoucang":ring_cur==4?"search":"setting");
        }
        ...
        /* B/CANCEL: 退出当前子视图返回主菜单（P1-2 复位环到 0=menu） */
        if ((pressed & MENU_CANCEL) &&
            (gl_showing || gl_searching || gl_setting || gl_typing ||
             gl_recent || gl_shoucang)) {
            ring_cur = 0;
            RING_APPLY();
            LOG("main_menu: back to main menu (ring reset -> 0)");
        }
        ...
        if (game_list_is_loaded() && game_list_count() > 0) {
            ...
            if (gl_showing) {
                ...
                /* 启动游戏（按 A/OK）— P1-3 起抽成 helper，TYPE 桶内共用 */
                if (pressed & MENU_OK) {
                    if (menu_launch_global(menu_gl_selected) == 0)
                        return;
                }
            }
        }
        ...
        /* ---- 调度 UI 模块 tick ----
         * P1-1/P1-3: 统一传**原厂事件码**（factory keycode bitmask，与 setting.xml
         * 热键同格式），各 UI 模块按 FK_* 位掩码判断。消除旧 keycode 约定
         * （0/1 与菜单数字 10-13）混乱 + 旧 bug（key_ok=1 在 setting 页误触 CANCEL）。 */
        ui_page_state_t cur_page = m_ui_current_page(
            gl_showing, gl_searching, gl_setting, gl_typing, gl_browser,
            gl_recent, gl_shoucang);
        uint32_t fk = keymask_to_keycode(pressed);
        m_ui_dispatch((int)fk, cur_page);

        /* ---- P1-3: TYPE 桶内启动检查 ----
         * ui_type_pending_launch 取出本地 bucket_sel 后，用
         * game_list_dir_index(ui_type_selected(), bucket_sel) 解析为全局索引，
         * 再走 menu_launch_global 完成 core_load。 */
        {
            int local_sel = ui_type_pending_launch();
            if (local_sel >= 0) {
                int bucket = ui_type_selected();
                int gidx = game_list_dir_index(bucket, local_sel);
                if (gidx >= 0) {
                    LOG("P1-3: TYPE bucket %03d local=%d → global=%d, launching",
                        bucket, local_sel, gidx);
                    if (menu_launch_global(gidx) == 0)
                        return;
                } else {
                    LOG("P1-3: TYPE bucket %03d local=%d → no game (empty), stay",
                        bucket, local_sel);
                    /* 桶内空，回退分类列表态 */
                    ui_type_clear_pending();
                }
            }
        }
        ...
        if (disp_is_ready() && ui_is_ready()) {
            if (gl_showing) {
                ...
                /* P1-3: TYPE 页 — 分类态画 9 桶 / 桶内态画桶游戏列表
                 * （ui_type_draw 内部已含背景绘制，无需再调 ui_draw_page） */
                ui_type_draw();
            } else if (gl_recent) {
                /* P1-2 最近页：用模块 draw（复用 type.raw 背景，ui_recent_draw 内含） */
                ui_recent_draw();
            } else if (gl_shoucang) {
                /* P1-2 收藏页：用模块 draw（复用 search.raw 背景） */
                ui_shoucang_draw();
            } else if (gl_browser) {
                ui_draw_page(UI_PAGE_GAME);
                if (fb_count == 0) {
