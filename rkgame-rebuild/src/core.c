...
int core_run(void)
{
    ...
    while (1) {
        ...
        /* P0-C: 检查暂停菜单热键（<gamemenuhotkey>，P1-1 修复）
         * 设备 setting.xml 存的是**原厂事件码 bitmask**（如 9=0x9=SELECT|START），
         * 旧代码 1u<<hotkey 会把 9 误解为 KEY_R2(1<<9=512) 的索引，完全错位。
         * 修复：本地 joy state → factory keycode 后做全包含 AND 检测。 */
        {
            uint32_t cur = keymask_to_keycode(joy_all_keys_state());
            uint32_t target = (uint32_t)g_cfg.gamemenuhotkey;
            if (target &&
                ((cur & target) == target) &&
                !(last_gamemenu_key & target)) {
                paused = !paused;
                LOG("gamemenuhotkey: factory=0x%x, pause %s",
                    target, paused ? "ON" : "OFF");
            }
