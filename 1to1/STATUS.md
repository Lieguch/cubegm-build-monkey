...
### 第四十九轮：**起始屏幕根因定案 + 场景 G/H 上线 + 差分公平修正**

| 项 | 第 48 轮 | **第 49 轮** |
|---|---|---|
| ★ 静态硬门禁 | 15 | 15（本轮改的是**场景设施**，未新增静态门禁） |
| 行为场景 | 5（A/B/C/E/F） | **7（A/B/C/E/F/G/H）** |
| 起始屏幕 | 未知（实测恒停在「设置页」） | **已知 = `menu.log` 头 u32；可控（0..5）** |
| 专有函数覆盖 | 68/223 = 30.49% | 68/223（本轮修的是"**为什么不动**"） |
| 已修缺陷 | 4 处 | **+2**（E/F 探针集不一致、注入按键语义无效） |
| 制品 | `build/qemu_art69`（316 文件） | 见提交 |

**根因（一句话）**：`main_Menu` 的分派是 `switch(menulog[0])`，而 `golden/sdcard_min/menu.log`
头 4 字节 = `05 00 00 00` ⇒ 开机即进 `mui_setting()` 的 `while(true)` 轮询
⇒ 表现为 `exit=124`（活着）但覆盖率**恒定** 68/223。

**新设施**：`CGM_MENULOG_SCREEN=<0..5>`（只改写头 4 字节，**默认关**，两侧共用，宿主侧生效）。
**新场景**：G（screen 0，vs **E** 单变量）／H（screen 0 ＋ DOWN/A 注入，vs **G** 单变量）。
**验证**：`sh -n` ✓、YAML ✓（15 步）、续行链 lint 0 问题、本地干跑三情形（未设/设 0/设 9）全对。

### 第四十九轮·补：**场景 G/H/I 首跑 + 起始屏幕/root.dat 两个根因**

| 项 | 数值 |
|---|---|
| 行为场景 | **8（A/B/C/E/F/G/H/I）** |
| 起始屏幕 | **可控（`CGM_MENULOG_SCREEN=0..5`）** —— 实测原厂 `menu.log` 头 u32 = 5 |
| root.dat | **可控（`CGM_ROOTDAT=1` 合成 ZIP）** —— 实测缺失导致两侧同形态崩溃 |
| 首次打开的路径 | `mui_menu`(3084 B) ＋ `mui_do_file_list`(1380 B) |
| 崩溃点 | `mui_do_file_list + 0xf0`（`ldrb r0,[r0]`，r0=0）⇒ NULL 传参 |
| 专有函数覆盖 | E/F 68/223（30.49%）；G/H **59/223**（换路，非退步） |
| 仪器/流程缺陷修复 | 3 处（短 sha 静默空集 / 探针集不一致 / 重试导致副作用执行两次） |
| 提交 | `4bd49241`（第 49 轮主体）+ 本轮补充提交 |
