...
⇒ **产物 = 「重建的 743 个函数（1.32 MB）」+「工厂机器码镜像（3.90 MB）」的混合体**，
   且为「逐位复刻工厂绝对地址」而把只读/可写区在地址上交错 ⇒
   **数学上不可能再得到原厂的 2 段连续装载结构**（原厂 `[R-X]`+`[RW-]`）。
   **而「能不能被加载/跑起来」恰恰依赖装载结构** ⇒ 这个架构在对抗它自己。

⇒ 那 9,997 个引用的来源已定位：**92 个工厂函数未重编** ⇒ 我们的代码只能 `bl` 到嵌进来的工厂机器码。

### 正确的判据（已做成可复现工具 `tools/fidelity_audit.py`）
| 刻度 | 现值（2026-09-22） | 目标 |
|---|---|---|
| **A. 工厂镜像引用数**（自有 `.text` → `.fimg_*` 的地址常量数） | **9,997** | **0** |
...
### 正确的路线（**收敛顺序**）
1. **补齐那 92 个未重编函数**（它们是 8,672 处引用的唯一来源）；
2. 引用数逐步下降 → 0 ⇒ **`.fimg_*` 整体可移除** ⇒ 只读区/可写区自然连续
