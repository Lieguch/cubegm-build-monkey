...
### 3.3 P2-A CI 实测结果（run #2，2026-09-10）★ 关键前提发现

`tools/ci_p2a_stb.py` 在 CI（Ubuntu GCC 11.4.0 / binutils 2.38）编译 v1.21–v1.26 并与金标准比对：

| 版本 | 候选函数数 | 覆盖 | 尺寸全等 | 尺寸±10% | 平均尺寸偏差 |
|---|---:|---:|---:|---:|---:|
| v1.21 f7d1cd58 | 97 | 77/81 | 6 | 11 | 49.6% |
| v1.22 787f1d64 | 97 | 77/81 | 5 | 11 | 49.6% |
| v1.23 7a69424f | 102 | 77/81 | 5 | 11 | 49.6% |
| v1.24 e140649c | 102 | 77/81 | 5 | 11 | 49.6% |
| v1.25 be901954 | 102 | 77/81 | 5 | 12 | 49.7% |
| v1.26 6e9f34d5 | 102 | 77/81 | 5 | 12 | 49.7% |

**结论（证据驱动）**：
1. 平均尺寸偏差 **≈50%**，且所有版本几乎不可区分 → **编译器版本差异（GCC 11 vs 工厂 GCC ~6）淹没了版本差异**。
2. → **P2-A 的前置条件 = 先取得原厂同款工具链** `armv7a-libreelec-linux-gnueabi`
   （LibreELEC/Lakka 8.0 系，glibc 2.24）。取得后可复跑本作业定版。
3. 覆盖 77/81 而非 81/81：原厂 4 个函数名带 `.isra/.part/.constprop`（GCC 产物后缀），
   不参与绝对比对；已归一化处理。

> **假绿教训**：首版作业 `objdump -t` 解析把尺寸索引写成 `p[1]`（实为绑定属性 `l/g`），
> `int('l',16)` 抛异常被静默吞掉 → 提取恒为 0，而作业仍报 success。
> **已加硬门禁**：覆盖全为 0 → `exit 1`；并加取证转储。修复后尺寸索引改为 `p[4]`。
