...
def main():
    ...
    B(' * ★★ 但 _init/_fini 连 PROVIDE 兜底也不能留：zig 无 crti.o 时会落到 0')
    B(' *   ⇒ DT_INIT=0 ⇒ glibc call_init 跳地址 0 ⇒ 任何输出之前就 SIGSEGV。')
    B(' *   正确做法：由 src/compat/crt_init.S 提供真实 .init/.fini 节。 */')
    B('/* _init/_fini 由 src/compat/crt_init.S 真实提供（见该文件说明）。')
    B(' * 此处**故意不再置 0**：漏链 crt_init.o 时要**链接报错**，而不是静默生成 DT_INIT=0。 */')
    B('')
