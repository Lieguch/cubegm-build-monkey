#!/bin/sh
# ============================================================
# build_probe2.sh —— 构建**探针 v2**：动态加载失败的定位器
#
# 与 build_probe.sh 的差别：v2 具备 fork+execve+wait4 能力，
# 能对一批候选二进制逐个测出「内核拒载 / ld.so 失败 / 程序自崩」。
#
# 仍然保持"最小"纪律：`-nostdlib -static -no-pie`
#   ⇒ 无 `.interp`、无 NEEDED、无 DT_INIT、无构造器；只经 `svc #0`。
#   ⇒ 它能在设备上跑起来这件事**已经被 v1 实测证明**（同一套技术路线）。
#
# 用法: CC="<zig> cc" sh tools/build_probe2.sh [输出路径]
# ============================================================
set -u
ROOT="$(cd "$(dirname "$0")/.." && pwd)"
CC="${CC:-arm-linux-gnueabihf-gcc}"
PY="${PY:-python}"
OUT="${1:-$ROOT/build/rkgame.probe2}"

winpath() { if command -v cygpath >/dev/null 2>&1; then cygpath -m "$1"; else printf '%s' "$1"; fi; }
D="$ROOT/build/probe2_obj"
mkdir -p "$D"
...
want = {2: 'fork', 3: 'read', 4: 'write', 6: 'close', 11: 'execve', 19: 'lseek',
        20: 'getpid', 37: 'kill', 114: 'wait4', 120: 'clone', 162: 'nanosleep',
        248: 'exit_group', 322: 'openat', 5: 'open'}
