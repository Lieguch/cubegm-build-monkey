#!/bin/sh
# ============================================================
# build_probe5.sh —— 构建**探针 v5**：v3 的现场抓取 + 栈回溯/指令字节（修 v3 的 maps 截断）
#
# v5 = v4 的能力 + 把写入次数降到个位数 + 修掉 v3 两个**探针自身**的缺陷（见 src/probe/probe5.c 头注释）。
...
#
# 用法: CC="<zig> cc" sh tools/build_probe5.sh [输出路径]
# ============================================================
set -u
ROOT="$(cd "$(dirname "$0")/.." && pwd)"
CC="${CC:-arm-linux-gnueabihf-gcc}"
PY="${PY:-python}"
OUT="${1:-$ROOT/build/rkgame.probe5}"

winpath() { if command -v cygpath >/dev/null 2>&1; then cygpath -m "$1"; else printf '%s' "$1"; fi; }
D="$ROOT/build/probe5_obj"
mkdir -p "$D"
