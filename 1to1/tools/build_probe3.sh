#!/bin/sh
# ============================================================
# build_probe3.sh —— 构建**探针 v3**：抓崩溃现场（ptrace 版）
#
# v2 的能力：fork+execve+wait4 ⇒ 分开「内核拒载 / ld.so 失败 / 程序自崩」。
...
PY="${PY:-python}"
OUT="${1:-$ROOT/build/rkgame.probe3}"

winpath() { if command -v cygpath >/dev/null 2>&1; then cygpath -m "$1"; else printf '%s' "$1"; fi; }
D="$ROOT/build/probe3_obj"
mkdir -p "$D"
...
$CC $ARCH -c -Os -w -ffreestanding -fno-stack-protector -fno-builtin \
    "$(winpath "$ROOT/src/probe/probe3.c")" -o "$(winpath "$D/probe3.o")" \
  || { echo "!! probe3.c 编译失败" >&2; exit 5; }
$CC $ARCH -c -w "$(winpath "$ROOT/src/probe/start.S")" -o "$(winpath "$D/start.o")" \
  || { echo "!! start.S 汇编失败" >&2; exit 6; }
echo "    probe3.o $(wc -c < "$D/probe3.o") B / start.o $(wc -c < "$D/start.o") B"

echo "== [2/3] 链接（静态、无 interp、入口 _start）=="
$CC $ARCH -nostdlib -static -no-pie \
    -Wl,-e,_start -Wl,--build-id=none \
    "$(winpath "$D/start.o")" "$(winpath "$D/probe3.o")" -o "$(winpath "$OUT")" \
  || { echo "!! 链接失败" >&2; exit 7; }
echo "    产物 $(wc -c < "$OUT") B"
