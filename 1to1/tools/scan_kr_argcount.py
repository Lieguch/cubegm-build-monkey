...
"""
# ============================================================================
# ★★★ 现状（2026-09-18）：**本工具已降级为「本地工作清单生成器」，不再作为 CI 门禁。**
#   原因：机器码口径（数 `bl` 前写过几个 `r0..r3`）有三类**结构性噪声**，取 max 会把噪声放大
#   —— 实测曾据此报出"19 个函数漏参"，跨调用者复核后**绝大多数是假阳性**（详见 GAP.md G1·补四）。
#   噪声来源：
#     · 临时寄存器当参数（`ldr r3,[r4,#204]`）⇒ 多算；
#     · 尾调用继承 r0（`mxmlRelease → mxmlDelete`）⇒ 少算；
#     · 编译器为未使用的形参预置 0（`stbtt_GetFontVMetrics(font,&asc,0)` 置 r3）⇒ 多算。
#   ⇒ **现行 CI 门禁请用 `tools/scan_call_args.py`**（以工厂 per-function 反编译 C 的调用表达式为准，
#     误差方向只少算不多算 ⇒ 无假阳性）。本工具保留用于**机器码层面的交叉巡检与取证**。
# ============================================================================
