...
// by Lucian Wischik to simplify its use in Windows/C++. Also encryption
// and unicode filenames have been added.
...
HZIP OpenZip(HANDLE h, const char *password);
HZIP OpenZip(const TCHAR *fn, const char *password);
HZIP OpenZip(void *z,unsigned int len, const char *password);
// OpenZip - opens a zip file and returns a handle with which you can
// subsequently examine its contents. You can open a zip file from:
// from a pipe:             OpenZip(hpipe_read,0);
// from a file (by handle): OpenZip(hfile,0);
...
ZRESULT UnzipItem(HZIP hz, int index, HANDLE h);
ZRESULT UnzipItem(HZIP hz, int index, const TCHAR *fn);
ZRESULT UnzipItem(HZIP hz, int index, void *z,unsigned int len);
// UnzipItem - given an index to an item, unzips it. You can unzip to:
// to a pipe:             UnzipItem(hz,i, hpipe_write);
// to a file (by handle): UnzipItem(hz,i, hfile);
...
HZIP OpenZipU(HANDLE h, const char *password);
HZIP OpenZipU(const TCHAR *fn, const char *password);
HZIP OpenZipU(void *z,unsigned int len, const char *password);
