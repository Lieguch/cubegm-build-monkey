...
## 16.99 ★★★★★ 「工厂数据引用被 -Os 整体消除」—— 一类**静态门禁完全看不见**的缺陷

**发现路径**：为了核实刻度 A 的真值（源码里有 13 个 `DAT_003b01xx` 被引用，却在产物中
出现 0 次），做了一次**单变量实验**：同一份源、同一 zigcc、同一 CFLAGS，**只改优化级别**。

```
src/proprietary/core/FUN_002b6c14_FBA_Load.c
...
**根因（Ghidra 输出形态在 C 里的必然结果）**：工厂在栈上是一个 **10 槽指针数组**
（`-0x150 … -0x12c`，40 B 连续，逐槽赋值 10 个候选 core 路径）。Ghidra 按**类型推断**
把它拆成 10 个独立局部量：
...
在 C 里「取局部标量的地址再 +1」是**未定义行为**，GCC/clang 允许假定"越界指针不被解引用"，
于是把后续槽的**全部写入删除**。后果：原厂「10 条候选路径」的链**退化成 1 条**。

**为什么这是最危险的一类**：产物**能编译、能链接、能启动**；符号不是"未定义"而是"少了一个"⇒
`link_audit`（重复定义/MISSING）、ABI、布局、`prop_equiv`、PT_LOAD 几何**全部保持绿色**。

**修法**（`src/proprietary/core/FUN_002b6c14_FBA_Load.c`）：按工厂**真实结构**还原成
`char *core_path[10];` —— 每个槽的偏移与总字节数逐字节不变，步进因此合法，10 条路径全部保留。
验证：修复后 `-O0` 与 `-Os` 的工厂数据引用**都是 11 个，Δ = 空**。

**连锁（反而证明门禁链是自洽的）**：源码修好后这 9 个符号变成**活引用**，而它们在
`src/data/factory_image.S` 里**没有别名**（该文件生成于 09-14，当时 `--missing` 名单为空）
⇒ `link_audit` 立刻报 **MISSING 9**、`link_full` rc=1 —— **红得正确**。
补供应（重跑 `tools/regen_data.sh`，别名 1097 → **1109**）后：MISSING 0、`link_full` rc=0、
**10 个地址全部物化 10/10**、产物 5,663,928 → **5,664,348 B**、`prop_equiv ★FAIL 0`、其余门禁全 0。

**新门禁**：`tools/dce_ref_diff.py`（判据 = Δ = `UNDEF(-O0)` 减去 `UNDEF(-Os)` 的工厂符号）。
自证 9 条（正例「拆局部量+跨对象步进」必须报 Δ；反例「同数组内步进 + 运行时下标」必须 Δ=0；
反例「不存在文件 ⇒ 不可判」；回归「已修文件必须 Δ=0」）。

---

## 17.00 ★★★★ Δ 是「待核清单」**不是**「缺陷证明」—— 存量 13 个文件的裁定与债务表

全量扫描 213 个文件（编译失败 0）：**13 个文件有 Δ**。已判定的两类**都真实存在**：

| 类型 | 判据 | 实例 |
|---|---|---|
| **UB-DCE（真缺陷）** | 重建源码里该局部量**经跨对象步进被读** | `core/FUN_002b6c14_FBA_Load.c` —— **已修**，Δ 归零 |
| **合法 DCE** | 重建源码里该局部量**只赋值、从不读** | `mui/FUN_00023204_mui_menu.c`：`local_32c`（第 32 行声明、第 168 行 `local_32c = (&DAT_003b2320)[iVar5 * 0x101];`）**全文无其它用处** ⇒ -Os 删它是**正确的** |

⇒ **纪律**：Δ 非空 ⇒ 必须逐个判定；**不能**因为"-Os 删了"就一律当缺陷，也**不能**一律当合法优化。

**债务表（必须缩小，登记在 `tools/dce_ref_diff.py` 的 `ALLOW`）**：

| 文件 | Δ 符号 | 备注 |
|---|---|---|
| `input/FUN_0000b014_TestUSBJoy.c` | DAT_003af00c/010/014 | 待核 |
| `misc/FUN_0002187c_DisplayPage_list.c` | DAT_003af2a0 | 待核 |
| `mui/FUN_0001bf80_mui_DisplayGameSum.c` | DAT_0020202d | 待核 |
| `mui/FUN_00023204_mui_menu.c` | DAT_003b2320 | **已判定合法 DCE** |
| `mui/FUN_00023e10_mui_type.c` | DAT_003b2320 | 待核 |
| `mui/FUN_00025094_mui_search.c` | DAT_003b2320 | 待核 |
| `mui/FUN_000277bc_mui_recent.c` | DAT_003b2320 | 待核 |
| `mui/FUN_00028a74_mui_shoucang.c` | DAT_003b2320 | 待核 |
| `mui/FUN_0002d0d4_mui_joystick_setting.c` | DAT_003af820/824/828/82c | 待核 |
| `mui/FUN_0002e040_mui_video_setting.c` | 同上 | 待核 |
| `mui/FUN_0002eac8_mui_load_state.c` | DAT_003af820/824 | 待核 |
| `mui/FUN_0002f320_mui_save_state.c` | DAT_003af820/824 | 待核 |
| `mui/FUN_0002ff8c_PauseMenu.c` | DAT_003af820/824/82c | 待核 |

★ 两条共享模式值得优先查：`DAT_003b2320`（5 个 mui 文件，读法都是 `(&DAT_003b2320)[i * 0x101]`，
步长 0x101 = 257 ⇒ 疑为 257 字节记录表的首字段）与 `DAT_003af820/824/828/82c`
（一个 4 元素 int 数组 = 屏参，被读进 4~6 个独立局部量）。
**判定入口**：到**原厂反汇编**核对该槽/该数组元素是否被读；重建源码"只赋值不读"就是**丢了这次读取的用途**。

---

## 17.01 ★★★★ 数据模块「重生成」是**会静默回退手工修复**的地雷 + `factory.ld` 契约门禁

`tools/regen_data.sh`（= `gen_data_module.py` + `gen_local_alias.py`）会**重写**已提交的
`linker/factory.ld` 与 `src/data/factory_image.S`。而这两份文件含**多次手工修复**，生成器若没跟上：

| 地雷 | 症状（本次实测复现） |
|---|---|
| **GAP 16.69 回退** | regen 后 `.ld` 少 33 行、多 4 行 —— `.data/.bss/.text` 又被钉回 `0x01000000/0x02000000/0x05000000` ⇒ 真机 exec 失败、`_diag/` 零日志（2026-09-21 已实测过一次） |
| **GAP 16.71 回退** | RELRO 族（`.data.rel.ro`/`.got`/`.dynamic`）被并回 `.data`、`.data` 不再页对齐 ⇒ `PT_GNU_RELRO` 覆盖 `.data` ⇒ 写全局变量即 SIGSEGV |
| **别名塌缩** | `--missing` 若误用**分类后的 MISSING**而非**全量 UNDEF**，别名 934 → 17 |
| **CRLF** | Windows 上 `open(...,'w')` 把 .S/.ld 写成 CRLF（实测 `factory_image.S` CR=2280），仓库里是纯 LF |

**修法（本次）**：① 生成器的 ③ 运行时区改成与 16.69/16.71 修复版**逐字节一致**；
② 所有文本产物强制 `newline='\n'`。
**验证（幂等性）**：修后重跑 `regen_data.sh`，`linker/factory.ld` 与修复版 **diff = 0 行**，
`.S`/`.ld` 的 **CR 计数 = 0**。

**新门禁**：`tools/check_regen_contract.py` —— 不重跑 regen（CI 无工厂 ELF），而是断言已提交产物的
5 条不变量（无钉死地址 / RELRO 族独立且先于 `.data` + `.data` 页对齐 / 别名数棘轮 ≥1100 /
文本产物纯 LF / 关键小段存在）。自证 7 条（1 正例 + 6 反例）。**反证**：把 `0x01000000`
塞回 `.ld` 后，门禁报出 **2 项 FAIL**（16.69 回退 + `.data` 未页对齐）—— 与预期一致。

---

## 17.02 刻度 A 的**真值与口径更正**（推翻我上一轮写进 PROJECT-MEMORY 的 175）

| 口径 | 值 | 说明 |
|---|---|---|
| 二进制区间法（旧 `fidelity_audit.py`） | **10007** | **无效**：把 `.text` vaddr 当文件偏移 + 「4 字节值落在大区间」大量误报 |
| 源码级·原始（含注释） | **162 个 / 2539 处** | |
| 源码级·代码（去注释与字符串）★ | **162 个 / 2534 处** | 权威 |
| 我上一轮写下的 | 175 个 / 2548 处 | **错**：把 13 个「引用了却在编译期被消除」的死引用也算进去了 |

**分项对照（新口径）**：`.fimg_data 135 · .fimg_rodata 20 · .fimg_bss 5 · .fimg_text 2`
（`fimg_rodata`/`fimg_bss` 两项与旧值完全一致，这也反证了统计方法本身没跑偏）。
★ `.fimg_text` 段内**只有 2 个 `UNK_` 符号**（`UNK_000d2f00`、`UNK_00118000`；第 3 个是容器符号
`__f_text_base`）⇒ 「几乎不执行工厂机器码」这个结论**更强**，不是更弱。

**新仪器**：`tools/fidelity_source_deps.py` —— 符号清单以**链接产物 ELF 的 `.fimg_*` 段符号**为准
...
## 17.05 ★★★★★ P2 判决：`FBA_Load` 的 10 槽还原**是对的**，size 比 1.356 = codegen

**问题**：我上一轮把 10 个独立局部量合并成 `char *core_path[10]`（消 UB），修后 size 比升到
工厂 708 / 我们 960 = **1.356**，刚过 p95(1.33) ⇒ 必须确认"还原是否过头"。

**判据（机器码级，权威 = 工厂自身）：**

```
工厂 FBA_Load @0x2b6c14  size=0x344
  2b6c28  sub  sp, sp, #0x13c            帧 0x13c + 9 寄存器 = 0x160
  2b6c30  ldm  ip!, {r0,r1,r2,r3}        ★ 从 GOT 连读 4 个指针
  2b6c34  add  r5, sp, #0x10             ★ r5 = 数组基址 = sp+0x10
  2b6c4c  stm  lr!, {r0,r1,r2,r3}        ★ 批量存 4 字 → sp[0x10..0x1f]
  2b6c50  ldm  ip!, {r0,r1,r2,r3}
  2b6c54  stm  lr!, {r0,r1,r2,r3}        ★ 再 4 字 → sp[0x20..0x2f]
  2b6c58  ldm  ip,  {r0,r1}
  2b6c64  stm  lr,  {r0,r1}              ★ 再 2 字 → sp[0x30..0x37]   ⇒ 恰好 **10 字**
  ...
  2b6d5c  ldr  r3, [r5, #4]!             ★★ 前索引写回：r5 每次 +4 ⇒ 逐个步进遍历 10 槽
  2b6db0  ldr  r3, [r5, #4]!
```

⇒ 这就是 Ghidra 渲染成 `ppcVar4 = &local_150; ppcVar4 = ppcVar4 + 1` 的那个"跨对象步进"的
**机器码原形**：工厂确实一次写 10 个字并逐个遍历 ⇒ **10 槽是真实结构，我的还原没有过头**。

**size 差来自 codegen（可量化）**：
...
工厂用 `ldm ip!` + `stm lr!` **批量搬运 10 个 GOT 指针**（6 条指令搞定），我们的 clang 面对
10 个具名符号只能**逐个 `ldr`/`str`**（21 条读取 + 6 条存栈）⇒ 指令数 +19、字面量池更大。
**这是两条工具链的搬运风格差异，不是语义差异。**

**★ 更强的判据（本轮新增）**：`tools/diff_exec.py` 差分执行结论 = **PASS**
（同停止类别 / 同返回值 / 同外部调用序列 / 同数据读写地址与宽度 / 同数据终值）。
⇒ 从此 P2 类问题**不再需要人读反汇编**。

## 17.06 ★★★★★★ 根本性转折：**逐函数差分执行**（differential execution）

**为什么做**：在此之前，判定"重建的 C 与工厂二进制语义是否一致"靠**人读反汇编 + 手工数据流
推断**。这条路不收敛：213 个函数、每次只看一条线索；更糟的是**仪器缺陷会被算成源码缺陷**
（实测：本轮 P1 的 3 个"发现"里 2 个是仪器错）。用户直接点出这是"穷举试错"。

**联网核实的根本方法**（官方文档原文）：Unicorn = 从 QEMU 提取的多架构 CPU 模拟器，
Python 绑定，支持 `UC_HOOK_CODE` / `UC_HOOK_MEM_READ/WRITE` **细粒度插桩**，官方定位即
"**Deterministic function-level analysis** / Unit testing assembly routines"。

**本项目的两个特殊便利（决定了可行性）**：
1. 两个二进制都是 `ET_EXEC`，且我们的产物把**工厂的 `.data`/`.bss` 放在同一批 vaddr**
   （数据区起始 **0x3ae5c4** 两侧一致）⇒ **全局变量地址一致 ⇒ 内存可逐字节直接对拍**；
2. 工厂二进制**未 strip**（`.symtab` 完整，814 FUNC / 956 OBJECT）⇒ 函数名/大小/数据符号可得。

**比什么（判据）**：
| 比 | 为什么 |
|---|---|
| 停止类别（return / 哪种 uc-error） | 一侧完成、另一侧崩 ⇒ 强发散信号 |
| 返回值 r0（**仅两侧都正常返回时**） | 异常停下时 r0 是残留值，拿它比会假发散 |
| 外部（PLT）调用序列 | 直接回答"是不是少实现/多调" |
| 数据区访存指纹 `(地址, 宽度, 读/写)` | **宽度窄化**（真机 SIGBUS 那类）在此现形 |
| 数据区最终内容（按工厂符号名） | 语义终态 |

**不比**（低信号、布局相关）：机器码字节、指令数、栈帧布局、内部函数调用序列
—— 工厂 GCC 6.2 与我们的 clang 必然在这些量上发散，**它们不是判据**。

**实测基线（746 个共有函数，每函数 3 组输入，3000 步上限，全程 1 分钟）**：
见 §17.07。

**仪器自身的 6 个缺陷（全部实测暴露并修掉，这是本轮最有价值的部分）**：

| # | 缺陷 | 症状 | 修法 |
|---|---|---|---|
| 1 | `s['st_name']` 取的是 **strtab 偏移(整数)** | 符号表建成 `{偏移: (地址,大小)}` ⇒ 函数名永远匹配不上，`--fn` 全部 SKIP | 改用 **`s.name`** |
| 2 | Unicorn `mem_map` **与已映射页重叠即整体失败** | 我们产物 9 个 PT_LOAD 中 3 段跨页 ⇒ 半段未映射 ⇒ `WRITE_UNMAPPED` | 先**合并页区间并集**一次映射，再逐段写内容 |
| 3 | 按**符号名**比访存 | 同一 vaddr 两侧名字不同（工厂 `m_ui` vs 我们 `DAT_003af2b4`）⇒ 几十条假发散 | 改为**按地址比**（地址两侧一致，本来就不需要名字） |
| 4 | 比较范围取"整个可写段" | 工厂 `.got`(0x3b1cfc) 在段内、我们 `.got`(0x4e13c8) 在段外 ⇒ 工厂 GOT 读被计入、我们的被过滤 | 收紧为**工厂具名数据对象的区间并集**（GOT/link 元数据天然排除） |
| 5 | 局部变量**覆盖了过滤参数**（同名 `spans`） | 过滤静默失效 | 改名 `pages`，并在注释里写明"这里改名是有原因的" |
| 6 | 未开 FPU | 我们产物含 VFP/NEON（`vpush {d8,d9}`）⇒ 只跑 3 条就 `INSN_INVALID` | **`CPACR=0xF00000` + `FPEXC=0x40000000` 两个都要**（只设 CPACR 无效，已最小复现） |

**★ 第 6 条的最小复现（同 4 条指令）**：
```
不加设置          → 执行 3 条即 UC_ERR_INSN_INVALID
CPACR=0xF00000    → 仍然 3 条即 UC_ERR_INSN_INVALID   ← 只设 CPACR 不够
CPACR+FPEXC.EN    → 正常执行
```

**判据升级（防"用假发散淹没真发散"）**：
* 地址类返回值（`_Z11zlibVersionv` 各返回自己的版本串地址）⇒ **比指向内容**（到 NUL 为止），不比数值；
* "工厂有调用我们没有、但**其余观测量全一致**" ⇒ 判 **INFO（内联/等价实现）**，不算发散；
* 多了调用 / 顺序变了 / 或其余观测量不一致 ⇒ 仍 **DIVERGE**。

**自证（12 条全过，且不依赖本项目数据）**：
用 zig 现编两个 ARM32 静态 ELF：同一份源码编两次 ⇒ 必须 PASS（顺带验证构建确定性）；
把循环里一个乘数 `3` 改成 `5` ⇒ 必须 DIVERGE **且差异定位到 `ret`**。

## 17.07 首轮普查基线与"发散"的真实构成

**基线（746 个共有函数，每函数 3 组输入，上限 3000 步，全程 53 秒）**：

```
汇总：PASS 591 | DIVERGE 155 | INFO(内联等价) 22 | SKIP 0
```

**收敛轨迹（仪器每修一个缺陷，数字就变一次 —— 这本身就证明"假发散淹没真发散"是最大风险）**：

| 阶段 | PASS | DIVERGE | 说明 |
|---|---|---|---|
| 首轮（未消仪器缺陷） | 577 | 169 | 假发散大量存在 |
| 修 #3/#4（按地址比 + 排除 `.got`） | — | — | 4 个抽查函数全转 PASS |
| 修 #5（局部变量覆盖过滤参数） | — | — | 过滤才真正生效 |
| 修 #1/#2/#6 + INFO 判据 | **591** | **155** | 当前基线 |
...
**★ 处置原则**：DIVERGE 是**待核清单**，不是"缺陷证明"——它可能来自
(a) 真缺陷、(b) 依赖外部环境（文件系统/设备节点/`dlopen`）、(c) 输入语料不含该路径、
(d) 尚未识别的仪器假象。**必须逐条定性后才能进结论**。

**已识别的"环境依赖"类（不进 1:1 判据）**：`TestUSBJoy` / `InitRFJoystick` / `JoystickTest` /
`GBC_Load` / `Gpsp_Load` / `FBA_Load` 等依赖 `/dev/*`、`dlopen`、真实 core 文件 ——
语料无法覆盖，其正确性由**真机 / 启动链**验证，不由本仪器。仪器对它们的价值是
"**两侧同类停下 + 同调用序列**"（这已是强等价证据）。

**优先核查队列（有 `stop` / `ret` 类差异 + 不依赖环境）**：
`ClearBuffer`、`Convert_Stereo`、`DequantBlock`、`MP3FindSyncWord`、`ReadPS2JS`、
`TurboKeyProcess`、`UpdateROM`、`_Z14timet2filetimel`、`GetFilenameExt`、`EmuCore_list`。

## 17.08 ★★★★ CI 红灯根因：门禁依赖了**本机专有输入**（第二次同类）

**现象**：`3f4ab810` 的 `1to1-verify` failure，报
`FileNotFoundError: D:/output/rkgame/decompiled/02-ghidra-c/03-per-function`。

**根因**：我新加的 `src_transcript_parity.py` 把语料路径**硬编码为本机绝对路径**；
而该语料（Ghidra 逐函数原始 C）**没有随仓交付**。这与 GAP 16.86 的
`.json` vs `.json.gz` 是**同一类病**：门禁依赖了"本地专有输入"。

**处置（按项目纪律仓内化，不是打补丁）**：
1. 语料打包入库：`golden/ghidra-perfn.tar.gz`（812 文件 / 3.3 MB → **253 KB**）。
   它是从 `golden/factory.rkgame.bin` 派生的**只读分析产物**，属"材料"不属"构建产物"。
2. 工具改为三级解析：`环境变量 GHIDRA_PERFUNC` → 仓内 tar.gz（自动解包到 `build/_ghidra_perfn`）
   → 本机开发路径；**全都没有 ⇒ 打印可执行的修复指引并 `exit 2`（fail-closed）**。
   ★ 明确**不做静默跳过**：静默跳过 = 假绿，正是本项目反复踩的坑。
3. **三态自证**（本地实测）：① 清缓存后能从仓内 tar.gz 自举；② **把本机开发路径改名后仍能跑**
   （= CI 近似态，即本次红灯的根因已消除）；③ 仓内也无时硬失败 `rc=2` 且给出修复指引。

**★ 另一个静默陷阱（与 CubeGM_RetroArch 的 `.gitignore` 吞 `driver.so` 同源）**：
`push_1to1.py` 的 `golden/` **刻意不参与 walk**（原厂只读资产、人工控制）⇒ 我加了
`golden/ghidra-perfn.tar.gz` 却**没有在 FILES 表里显式登记** ⇒ 第一次推送只上了 2 个文件、
tar.gz **静默没上**（VERIFY 显示 1301 个 blob 全部 match，看不出问题）。
⇒ 纪律：**凡往 `golden/` 加文件，必须同时在 `push_1to1.py` 的 FILES 表登记**。

**CI 结果**：`46937237` 三个 job **全 success**（`1to1-verify` 3.8 分 / `1to1-qemu-behav` 15.5 分 /
`rkgame-rebuild` 1.3 分）。

## 17.09 ★★★★ 差分器接进 CI（**棘轮台账**）+ 一条必须遵守的协作纪律

**CI 接线（`1to1-verify.yml`，现 38 步，两道新硬门禁）**：

| 步骤 | 作用 | 为什么必须分两步 |
|---|---|---|
| `差分执行器自证`（`--self-test`） | 用 zig 现场编两个 ARM32 静态 ELF 做正/反例 | **它是"尺子"**：尺子坏了，后面所有对拍结论都不可信 ⇒ 必须先单独证尺子 |
| `差分执行对拍`（`--batch --ledger`） | 746 函数 × 3 组输入对拍，**棘轮**只允许发散减少 | 现存 155 个发散是**待核债务**不是缺陷证明 ⇒ 不能直接红 |

**棘轮（ratchet）语义**：台账 `tools/diff_exec_pending.txt` 记录已知发散函数集。
* 出现**台账外的新发散** ⇒ `rc=2` 失败（"修一个坏一个"溜不过去）；
* 台账内条目**收敛** ⇒ 报告为 `已收敛`，不算失败（鼓励修）；
* 台账须用 `--update-ledger` 显式重写（留痕，不在 CI 里自动改）。

依赖入库：CI 里 `pip install unicorn==2.1.4 capstone==5.0.7 pyelftools`（与本地同版本）。

**★ 一条必须遵守的协作纪律（本轮实测的险情）**

推之前发现：**远端 HEAD 已推进到我之后的 `61eb0041`**，且本地出现了我这个上下文之外的
`tools/lint_ci_reach.py`（12:53）与 `tools/ci_local_paths_allow.txt`（12:49）
—— 说明**同一工作区在两次会话之间被继续推进过**（含把"硬编码本机路径"这件事**类级机械化**：
`lint_ci_reach.py` 会解出 workflow 的 CI 可达工具集，要求其中每个**驱动盘绝对路径**要么可被
环境变量覆盖、要么在注释里、要么有台账理由，**并对台账里的 `devonly:` 条目做反向自证**）。

**风险**：我的本地版本若落后于远端，一次 `push` 就会**静默覆盖别人的改进**。
**处置（今后一律照做）**：
1. **推前先对比远端同一路径的 sha256**（本地 vs `?ref=<远端 HEAD>`）；
2. 若不同，**必须做行级 diff 并确认"仅远端 0 行"**（即本地是严格超集）才允许推；
3. 结果留证：本轮 `diff_exec.py` 远端 29,080 B / 本地 32,286 B ⇒ diff 后确认
   **远端独有的行 = 0**（远端正是我上一版）；`GAP.md` / `PROJECT-MEMORY.md`
   **仅远端 0 行 / 仅本地 165+33 行** ⇒ 安全推送。
   ★ 这一条与"`golden/` 必须显式登记"是同一族纪律：**推之前先证明"我不会覆盖别人"**。

推送 `0b270901`（7 blobs，VERIFY 1306/1306）。

---

## 17.10 ★★★★★ 棘轮台账的**静默变弱**：TRUNC ≠ 已收敛 + 判据强度必须与台账声明一致

### A. 病灶（本轮实测，第 60 轮）

CI `1to1-verify@0b270901` 红在 `check_types.py`（旧版解析缺陷，已修）。按 CI 日志发现：
**`check_types` 之后的全部步骤在 CI 里从未执行过**（步骤顺序，第一步失败即整 job 停）。
⇒ 也就是说 `diff_exec --batch --ledger` 这条门禁**自上线以来一次都没在 CI 上跑过**。

于是按 CI 的**逐字参数**（`--batch --steps 3000`）在本地复跑，得到 rc=0 / ★新增 0。
**但 rc=0 是假的**：我随即抽查了"台账里被删掉的 14 个名字"，发现

| 函数 | `--steps 3000` | `--steps 20000` |
|---|---|---|
| `libiconvlist` | **TRUNC**（不可判） | **DIVERGE** |
| `xmp3_PolyphaseStereo` | **TRUNC**（不可判） | **DIVERGE** |
| `AudioProcess` | **TRUNC** | **DIVERGE**（放大后现形，见 D） |
| `TestRun` / `WaitNMI` / `MP3InitDecoder` / `xmp3_AllocateBuffers` | TRUNC | TRUNC（确实跑不完） |

⇒ **两个真分歧因为"跑不完"落进 TRUNC，就被 `--update-ledger` 当成"已收敛"从台账里删掉了。**
旧代码里 `fixed = [n for n in old if n not in div_names]` —— 它把 **TRUNC / SKIP 也当成收敛**，
还在报告里打印 `✓ 本轮收敛`。**这是本项目最难发现的一类缺陷：门禁主动把"未知"改写成了"没问题"。**

代价口径：`1to1-verify` 从 `46937237`（三 job 全绿）之后连续两轮红，其中一轮的"绿"是假的；
`cubegm-build-monkey` Actions 预算按 2000 分钟/月，这两轮 = 白烧。

### B. 根因（不是"阈值调小了"，是**三处语义缺失**）

| # | 缺失 | 后果 |
|---|---|---|
| 1 | **`TRUNC` / `SKIP` 被并入"已收敛"** | 不可判 ⇒ 静默删台账 ⇒ 判据变弱且不可见 |
| 2 | **台账不记录自己的判据强度**（`--steps` 只在命令行上） | 用 3000 步重写、用 20000 步评测（或反之）都能跑，**没有任何东西会报错** |
| 3 | **`TRUNC` / `SKIP` 名单不落盘** | 报告只列 DIVERGE 明细 ⇒ "没判"读起来像"没问题" |

### C. 修法（三处，全部是"把不变式变成机械判据"）

1. **棘轮语义写成纯函数** `ledger_update(old, diverging, undecidable)`：
   > 保留 = 本轮发散 **∪**（旧台账 ∩ 本轮**不可判**）；移除 = 仅"本轮被明确判为 PASS/INFO"的项。
2. **台账自描述 + 强度不一致即 fail-closed**：台账头部写 `# steps=3000` / `# escalate=30000`；
   评测时与命令行不符（或台账缺声明）⇒ **`exit 3`**，附"用同一强度重写台账"的修复命令原文。
3. **触上限者按 `10×` 预算自动重试一次**（`ESCALATE_FACTOR`，只对确实截断的函数付费）——
   把"不可判"从"常态"压缩成"例外"：本轮 TRUNC 由 10+ 降到 **4**，并**捞回 2 个真分歧**。
4. 报告新增 **TRUNC 名单**与 **SKIP 名单**两节（不可判必须列名）。

### D. 修后基线（判据强度已上移，**不是退步**）

| 口径 | PASS | DIVERGE | INFO | TRUNC | SKIP |
|---|---|---|---|---|---|
| 修前（steps=3000，无放大） | 600 | 140 | 22 | 6 | 0 |
| **修后（steps=3000 + 10× 放大）** | **600** | **142** | **22** | **4** | 0 |

台账 140 → **142 项**（新增 `AudioProcess`、`libiconvlist`）。
★ `AudioProcess` 是本轮**新增的真发现**（放大后才判得出）：我们侧会 `data-read 4070064` 并
`gettimeofday ×2`，工厂侧**两者都没有** ⇒ 我们多了一次时间读取 + 两次系统调用 ⇒ **待核项**。
剩余 4 个 TRUNC（`MP3InitDecoder` / `TestRun` / `WaitNMI` / `xmp3_AllocateBuffers`）
即"跑不完"函数，**按语义保留为不可判债务，不得计作已收敛**。

### E. 四态自证（全部实测，逐一留证）

| 态 | 构造 | 期望 | 实测 |
|---|---|---|---|
| ① 正常态 | 新台账（声明 steps=3000/escalate=30000） | rc=0 | **rc=0**，★新增 0 |
| ② 缺陷态·强度不一致 | 用**无声明**的旧台账（= 线上那一版） | fail-closed | **rc=3**，打印"台账声明 None / 本次 3000+30000"与修复命令 |
| ③ 缺陷态·台账混入不可判项 | 台账追加 `TestRun`（TRUNC） | 不得报"已收敛" | **rc=0**，`仍不可判（保留）1` + `⏸ 不得当作已收敛：['TestRun']` |
| ④ 缺陷态·台账漏登记 | 从台账删掉 `main_Menu`（DIVERGE） | 报新增发散 | **rc=2**，`★新增 1 ['main_Menu']` |

另：`--self-test` 由 13 条扩到 **26 条**，新增的 13 条全部针对台账纯函数
（解析 / 强度一致性 / 保留语义），**不依赖本项目数据** ⇒ 在 CI 里同样能跑。

### F. 纪律（本轮新增，写进"不要重复"清单）

> **1. 不可判 ≠ 已收敛。** 任何"未知"桶（TRUNC / SKIP / 超时 / 例外）**不得**被自动判据
> 当作通过，也不得被自动从棘轮台账里删除；它只能被**人工显式**结清。
> **2. 判据强度是判据的一部分。** 凡"重跑一次就能改变结论"的参数（步数 / 超时 / 样本数 /
> 输入组），必须**写进台账并被机械对拍**；强度不一致一律 fail-closed，不许只告警。
> **3. "没跑"与"跑了但过了"必须可区分。** 门禁的每个桶都要**列名落盘**；
> 只打印失败明细的工具，会把"没判到"伪装成"没问题"。
> ★ 这三条与 §2.5（七个仪器静默失效）、GAP 17.04（新判据首版必过严或过松）同族：
> **工具自己不做自证，它就是下一个缺陷的来源。**

### G. 本轮附带发现（治理）

`1to1-verify` 是**单 job 顺序执行 30+ 步**，**第一步失败 ⇒ 后面 20+ 步一次都不跑**
（本轮实测：`check_types` 一红，`diff_exec` 两步、`dce_ref_diff`、`regen_contract`、
`src_transcript_parity` 等全部从未执行）。⇒ **"CI 绿了"的信息量 = 最弱那一步的信息量**。
...
## 17.14 ★★★★★ 从「113 条发散」到「类别」：4 处仪器缺陷 + 一处**公开更正**

### A. 入口：先把"逐条发散"归约成"类别"（新工具 `tools/diverge_cluster.py`）

上一轮的教训是"一次根因能同时消掉 9 个函数"。⇒ 正确读法不是"113 个缺陷"，而是
"**113 个观测 → 若干类别**"。新工具做两件事：
1. 把差异文本里的裸地址/地址元组**符号化归一**（`<符号名>+<偏移>:<宽度>:<读写>@0x地址`），
   使"同一根因落在不同地址"能聚成类、"不同根因"不会误并；
2. 按归一签名分组，按组大小排序输出。

首轮结果：113 个发散 → **69 类**，最大一类 **20 个函数**，签名只有一行：
```
data-reads 仅F=[_mxml_key+0:4:R, _mxml_key+0:4:R] 仅O=[]
```
⇒ 全部是 mxml：**工厂读它自己的 `_mxml_key`（file-static，pthread TLS key），我方侧什么都没记**。

自证 **10 条**（含"同符号两地址前缀相同"、"异符号不合并"、"宽度/读写进键"、
"已知发散函数 `DequantBlock` 被归因到真符号"）。

### B. 根因（4 处仪器缺陷，**全部朝"假发散"方向**）

| # | 缺陷 | 实证 |
|---|---|---|
| **E1** | `_build_spans(bf)` **只取工厂**的具名数据对象 ⇒ **我方自己的数据对象整体在区外被过滤**，我方对私有副本的访问**完全不可观测** | mxml 的 file-static `_mxml_key`：工厂 0x3B1CF4 / 我方私有副本另一地址 ⇒ 工厂侧读被记、我方侧不记 ⇒ **20 个 mxml 函数假发散** |
| **E2** | 地址归因取"最近前驱"但**不跳过 Ghidra 合成名**（`DAT_xxxx`/`UNK_xxxx`）；这些名字是**我们造的**、工厂没有 ⇒ 一切都配不上对，还把真名盖掉 | `m_ui+80` 被 `DAT_003af29c` 之类盖成裸地址 `0x003af2b4` ⇒ `DisplayLine_list`/`EmuCore_Line`/`PauseMenu`/`IsShoucang` 等 **15+ 个函数假发散** |
| **E3** | **同名两份的判定错误**（我自己的两次纠错）：第一版用"名字级优先 LOCAL"的绑定表 ⇒ 把 `handle` 的 GLOBAL 那份也判成私有；第二版写"LOCAL 且无同名 GLOBAL"，而工厂的 `handle` **两份都是 LOCAL** ⇒ 仍判成私有 ⇒ 同一地址在两侧配不上 | `FBA_Load`/`Load_Proc2`/`retro_load_state`/`retro_save_state`/`run_process`/`LoadDefaultState` ⇒ **6 个假发散** |
| **E4** | 我方**无名区域**（如我们 `.bss` 头部的填充字节 0x4E3000）在工厂侧**没有对应区段**，按绝对地址比必然不对称 | `__libc_csu_init`（仅O 读 1 字节）/`_mxml_init`（仅O 写 1 字节）⇒ **2 个假发散** |

### C. 修法（4 处，都可机械判定）

| 修法 | 判据 |
|---|---|
| **spans 取两侧并集**（`_build_spans(bf, bo)`），且改用 `sym_list`（**保留同名多份**）而不是 `data_syms`（名字为键会折叠） | 我方私有对象的访问从此**可观测** |
| 归因**跳过合成名** `^(DAT\|UNK)_[0-9a-f]{6,8}$`（它们把地址编进名字、对跨侧配对零信息量） | 真名（`m_ui`）优先 |
| 访存指纹归一：**仅当"工厂里该名字唯一且为 `STB_LOCAL`"** 时按名字配对（`('LN', 名, 偏移, 宽度, 读写)`）；否则按**地址**（`('A', 地址, 宽度, 读写)`） | 唯一 ⇒ 地址是实现细节（私有副本合法）；两份 ⇒ 用哪份是语义 ⇒ 必须同址；GLOBAL ⇒ 地址是契约 ⇒ 必须同址 |
| `('A')` 形访问**必须落在"工厂侧有具名对象覆盖"的区段内** | 无对照区段（我方无名字节 / 工厂未命名字面量）⇒ 无观测力 ⇒ 不计入 |

★ 展示串**必须带绝对地址**（`m_ui+80:4:R@0x3af2b4`）：同名两份时只写 `m_ui+80` 读者无法判断是哪一份
—— 我自己就因为漏掉地址，把"同址"误读成"异址"，绕了一圈（见 E3）。

### D. 结果（**判据强度上移**，且新增发散为 0）
...
**30 个收敛项全部有机制解释**（不是"碰巧变绿"）：
* **mxml 家族 20 个**（`_mxml_global` / `mxmlLoad*` / `mxmlSAX*` / `mxmlSave*` / `mxmlEntity*` /
  `mxmlSet*` / `mxml_load_data` / `mxml_set_attr` / `mxml_error`）⇒ `_mxml_key` 私有副本按名字配对；
* **libiconv 转换器 8 个**（`gb2312_wctomb` / `gbk_wctomb` / `isoir165_wctomb` / `jisx0208_wctomb` /
  `jisx0212_wctomb` / `ksc5601_wctomb` / `tcvn_mbtowc` / `viscii_mbtowc`）⇒ 各自的转换表是**私有静态表**；
* **`DequantBlock` + `_Z6zErrori`** ⇒ 同上（私有表）。

### E. ★★★ 公开更正（SOUL：发现错就公开改，不悄悄改口）

**上一轮 GAP 记录的"差分器首个独有发现"是错的。** 原文大意：
> 「工厂 `DequantBlock` 读 `pow43_14`/`pow14` 反量化表，**我们完全不读**」⇒ 记为首条待核项。

**事实**：那是 **E1 造成的仪器盲区**——我方的反量化表是**我们自己的私有副本**（另一地址），
访问落在"工厂具名对象区间"之外 ⇒ **根本没被记录**，于是看起来"我们完全不读"。
**修掉 E1 后 `DequantBlock` 直接转为 PASS**（读的字节内容逐位一致）。
⇒ **教训：仪器造成的"缺失"和真实的"缺失"长得一模一样；凡"某侧完全没有某行为"的结论，
先证明该行为在本侧**可被观测**。** 这一条已加入纪律（见 F.9）。

### F. 附带修掉的一个"生成器元数据"缺陷：**别名继承基址符号的 `st_size`**

**症状**：我方 ELF 里 1117 个 `.set` 别名的 `st_size` **一律等于整段大小**
（`.fimg_bss` 194907 / `.fimg_rodata` 856920），而工厂的对应符号是 64 / 2944 / 4 …。

**单变量实验（三组对照，全部实测）**：
| 构造 | 别名 st_size |
|---|---|
| 基址**无** `.size`（只 `.space 0x100`） | `0` |
| 基址**有** `.size 0x100`，别名不写 `.size` | **256（继承基址！）** |
| 基址有 `.size`，别名显式 `.size 8` | `8` |
| 基址有 `.size`，别名显式 `.size 0` | `0`（**能覆盖**） |

⇒ 根因：**LLVM MC 的 `.set A, B + off` 会继承 `B` 的 `st_size`**，而 `gen_data_module.py`
给每个 `__f*_base` 都写了 `.size`（段大小）。

**修法**：`gen_data_module.py` 为**每个**别名补 `.size NAME, <真实尺寸>`（尺寸取自账本；
`DAT_/UNK_` 合成名与 `SPLIT_ALIASES` 显式给）。修后我方符号尺寸与工厂**逐位一致**：
`ArchivePath` 64 / `number` 40 / `key2` 28 / `GamePath` 28 / `m_ui` 1508 / `SoundBuffer` 2944 ✓。
再生成产物 diff：`factory_image.S` 仅多出 1117 行 `.size`；`factory.ld` **逐字节不变**；ELF 尺寸不变。
（修之前，一切"按名字归因"的工具都会指错对象 —— `diverge_cluster` 的首版就因此全错。）

### G. 纪律（新增第 8、9、10 条）

> **8. 只有"有对照"的观测量才有判据力。** 两侧各自私有的无名区域没有对应物，
> 拿它比绝对地址必然产生假发散；反过来，**只取一方的可观测范围**又会让另一方的行为
> 完全不可见（E1）。⇒ 区间取**并集**，配对**按语义**（唯一私有名按名字、其余按地址）。
> **9. "某侧完全没有某行为"的结论，必须先证明该行为在本侧可观测。**
> 仪器造成的"缺失"与真实的"缺失"长得一模一样（`DequantBlock` 一例）。
> ⇒ 判据要先证明**能看见**，再谈**看见了什么**。
> **10. 别名的元数据会从被引用符号"继承"。** `.set A, B + off` 继承 `B` 的 `st_size`；
> 凡"只分配一次、其余全是别名"的数据模块，**必须给每个别名显式写 `.size`**，
> 否则一切按名字/尺寸归因的工具都会指错对象。

---

## 17.15 ★★★★★ 第 64 轮审计：把「进度」拆成**能回答"还差多少"的四段**

### 0. 为什么要做这次审计

用户的要求是"用现在的进度与原厂 RKGAME 对比做一次审计"。此前项目的所有刻度
（`prop_equiv` 的 size_ratio / `diff_exec` 的 PASS 计数 / qemu 覆盖率）都只回答
"**某个代理指标**动了没"，**不回答"发散落在谁的代码里"**。而"落在第三方库"与
"落在我们自写的专有代码"是**性质完全不同**的两件事 —— 混在一起，就会把
"追着第三方库的版本差异打补丁"当成进度。

⇒ 新仪器 `tools/audit_vs_factory.py`（自证 14 条），固定输出四段：
**A 构建指纹 · B ELF 结构（契约 vs 实现细节）· C 覆盖（按来源分桶）· D 行为（按来源分桶）
+ E 发散→根因类别**。所有比例都同时给分子/分母；"未分类"桶**必须列名**，不得静默并入。

### A. 构建指纹 —— 决定「1:1 机器码保真」是否可达

| 维度 | 工厂 | 我方 | 后果 |
|---|---|---|---|
| 编译器 | **GCC 6.2.0** + **Linaro GCC 4.9.4**（两组对象混编） | **clang 21.1.0**（zig cc 0.16.0） | 每函数机器码必然不同；内联/特化/载入窄化策略不同 |
| 链接器 | **GNU gold 1.12**（`.note.gnu.gold-version`） | lld | 节区数 0x2a vs 0x1f、`PT_LOAD` 2 vs 9 |
| libc **构建头** | **glibc-2.24**（`.rodata` 里 `glibc-2.24/.armv7a-libree`、`glibc-2.24/csu`） | 设备 sysroot 2.29 / 工具链自带头 | ctype 宏展开、`_chk` 变体、`__strdup` 等符号名选择不同 |
| `DT_NEEDED` | 7 个（多 `libstdc++.so.6` / `libgcc_s.so.1`） | 5 个 | 已知可接受差异（我们不用 C++ 异常/RTTI） |
| 我方 `.comment` | — | **整体缺失** | 产物里**没有**编译器指纹 ⇒ 审计只能靠外部记录；已登记为待补元数据 |

★★ **结论（本次审计最重要的一条）**：**"1:1 机器码保真"在本项目里按构造不可达** ——
两侧跨编译器家族且跨 glibc 头版本。因此判据必须建在**行为/契约**上，而不是"指令序列像不像"。
这解释了为什么余下的差异里有相当一部分**永远修不掉、也不该修**。

### C. 覆盖（分母 = 工厂有名 FUNC 814；按**来源**分桶）

| 来源 | 函数数 | 字节 | 字节占比 | 已落我方 `.text` | 落位率 |
|---|---|---|---|---|---|
| 上游（第三方库） | 550 | 167,686 | 52.8% | 504 | 91.6% |
| **专有（需逐函数重建）** | **223** | **122,922** | **38.7%** | **217** | **97.3%** |
| 未分类 | 27 | 25,636 | 8.1% | 20 | 74.1% |
| 运行时/CRT | 14 | 1,132 | 0.4% | 5 | 35.7% |

> ★ 与旧的 `GAP-TO-REPLACEMENT.md`（"工厂有名 FUNC 804 / 专有 223 / 上游 581"）**对不上**：
> 差异来源：旧口径的"上游 581 / 61.3%"用的是**我们仓库里的源码函数数**，而本次是
> **工厂符号数**按来源归因 —— **分母不同**。本节的数字是**工厂口径**，以它为准。
> ★ 未分类 27 个已**全部列名**（`xmp3_*` 18 个 / `.isra`·`.part` 特化符号 5 个 / 其余 4 个）——
> 它们是"既不在专有清单、也不在上游标识符索引"的，属于**归因缺口**，已列出待补。

### D. 行为（可比函数 746；按来源分桶）

**当前基线：`PASS 665 | DIVERGE 75 | TRUNC 6`**（`--steps 3000`，void 函数 r0 不作判据）。
...
专有 29 个发散的分布（这是**真正要打的靶子**）：`mui` 最多（UI 引擎，也是 70,396 B 的主体）、
其次 `misc`，少数在 `core`/`flash`/`hw`/`input`。

### E. 发散 → 根因类别（可操作清单）

| 类别 | 个数 | 含义与修法 |
|---|---|---|
| `R-SET` | 23 | 读集合不同（一侧读、另一侧完全不读）⇒ 可能真缺失，也可能是内联/私有副本 |
| `CRASH` | **21** | **一侧崩、一侧正常返回** ⇒ 最优先（含我方 `UC_ERR_READ_UNMAPPED`） |
| `RET` | 18 | 返回值不同 ⇒ 真语义差异 |
| `W-RANGE` | 7 | 写覆盖区间不同（已做同向合并）⇒ 真丢失/多余的写 |
| `CALLONLY` | 4 | 仅外部调用集合不同 ⇒ 大概率纯函数被 CSE |
| `WIDTH-R` | 2 | 同址两侧都读、仅宽度不同 ⇒ 访存粒度（见 §17.17） |

---

## 17.16 ★★★★★ 外部调用一律返回 0 ⇒ **尺子看不见**（4 个假发散 + 18 个"看不见"）

### 症状

`diff_exec.run_func` 的 `code_hook` 对**所有**外部调用一律 `r0 = stub_ret.get(name, 0)`。
对 `int` 返回型这是"中性"的；对**指针返回型**它等于"返回 NULL"：

```
strupr         F=['islower']  O=['__ctype_b_loc']   ⇒ O 侧 UC_ERR_READ_UNMAPPED
get_from_line  F=['strlen']   O=['strchr']          ⇒ O 侧 UC_ERR_READ_UNMAPPED
myStrrstr / GetFilenameExt                          ⇒ O 侧 UC_ERR_READ_UNMAPPED
```

⇒ 违反本项目**纪律 9**：「某侧完全没有某行为」的结论，必须先证明该行为在本侧**可被观测**。
这 4 个函数被判成 DIVERGE，**是尺子的缺陷，不是代码的缺陷**。

### 根解：`tools/libc_model.py`（自证 32 条）

给差分器一个**真正的 libc 语义模型**，而不是"返回 0"：

* **两侧共用同一份模型与同一组地址**（CTYPE 表 @0x7F000000、模拟堆 @0x7F800000）
  ⇒ 指针可比、堆内容可比，差异只可能来自被测代码；
* 实现 `strlen/strchr/strrchr/strstr/strcmp/strncmp/strcasecmp/memcmp/memchr/
  strcpy/strncpy/strcat/strncat/strdup/strndup/memcpy/memmove/memset/
  malloc/calloc/realloc/free/atoi/strtol` + ctype 全家族 + `__errno_location`；
* **ctype 表按 glibc 文档的位定义构造**（`_ISbit(n)` 公式），并与谓词函数**交叉验证**；
* **未建模的调用必须登记**（`unmodelled`）并在报告里**列名**，绝不静默当成"一致"；
* 堆**不清零**（填 `0xA5`）—— 若清零，`malloc`/`calloc` 的语义差会被抹掉 ⇒ 假 PASS。

### ★★ 踩到的坑：**两份硬编清单必然漂移**（纪律 6/7 的重演，这次代价 = 26 个假发散）

`__strdup` / `_Znwj`（C++ `operator new`）加进了**报告侧的 `CALL_ALIAS`**，却漏加进
**模型的别名表** ⇒ 工厂侧"名字不认识 ⇒ 未建模 ⇒ 返回 NULL"，我们侧真执行
⇒ `OpenZipU` / `PauseMenu` / `mui_DisplayThumbnail` / `get_items_from_zipfile` 等
**一次造出 18 个假发散**（签名统一为 `F=UC_ERR_WRITE_UNMAPPED / O=return`）。

**修法（根解，不是补丁）**：`CALL_ALIAS = dict(libc_model.ALIASES)` —— **单一真源**，
并由自证断言 `CALL_ALIAS == libc_model.ALIASES`。

### A/B 单变量对照（证明模型只是"让尺子变准"，没有掩盖差异）

新增 `CGM_NO_LIBC_MODEL=1` 开关，同一工具、同一产物、**只差这一个开关**：

| | 开模型 | 关模型 |
|---|---|---|
| DIVERGE | **84** | 79 |

* **消除 3 个（原为仪器假发散）**：`strupr` / `TestUSBJoy` / `mui_recent`
* **新看见 8 个（原为"不可观测"）**：`mxmlSave{Fd,File,String,AllocString}` /
  `mxmlNewCDATA` / `mxmlNewTextf` / `mxmlEntityGetValue` / `TestLibz0`
  —— 其中 5 个是 **mxml 保存路径**，本项目**从未观测过**这段代码。

⇒ 净 +5，但构成变了：**−3 假发散，+8 真可见**。这类"分数下降其实是能力上升"的情形，
必须靠这种单变量对照来证明，而不是靠自述。

---

## 17.17 ★★★★★ 访问**粒度**不是语义（含一次**公开更正**）

### A. 写：`两个相邻 4B 写` == `一个 8B 写`

实测 `main`：工厂 `0x3e1298:4:W ×1` + `0x3e129c:4:W ×1`，我们 `0x3e1298:8:W ×1`
—— **同一区间、同一内容**，clang 把两次相邻 32 位存合并成一次 64 位存。
修法：`coalesce_writes()` 只对**写**做**字节覆盖区间**归一（同向、相邻/重叠才合并）。

### B. ★ 更正：读的宽度差异**也不是**"值不同"

我最初（本轮中途）把"同址不同宽"写成"工厂读 4B / 我们读 1B ⇒ **读到的值不同** ⇒ 真实声明缺陷"。
**那是错的，现公开更正。**

实证：`DisplayThumbnailflag` 在 `globals.h` 里声明为 **`unsigned int`（4 字节）**，
而**所有**用法只触及低字节（`& 8` / `& 0xfe` / `| 2` / `| 0xc`）⇒ clang 做
**load-narrowing**（只用到低字节 ⇒ 合法地把 4B 载入窄化成 1B）；GCC 6.2 没做
⇒ 工厂读 4B、我们读 1B。**同址、低位值相同 ⇒ 语义等价**，与 A 是同一类**访问粒度**差异。
⇒ **没有需要修的源码缺陷**（我差点为一个不存在的缺陷去"改声明"）。

**修法（保守，只放开这一类）**：`read_width_only()` —— 当且仅当
**对象与地址集合完全一致、唯一差别是每次读的宽度**时，降级为 **INFO** 并**列名留痕**
（`粒度等价（仅访存宽度不同、同址同对象）: …`）。地址集合不同（R-SET）照旧报发散。

**四条反向自证**（防这次放宽被滥用）：地址集合不同 ⇒ 不是粒度；次数不同 ⇒ 不是粒度；
方向不同 ⇒ 不是粒度；相邻读**不得**被写合并规则吸收。

---

## 17.18 本轮数字（同一把尺子的前后对比）

| 指标 | 第 63 轮末（旧尺子） | 第 64 轮（新尺子） |
|---|---|---|
| `diff_exec` 锚点自证 | 45 | **63** |
| `libc_model` 自证 | — | **32**（新） |
| `audit_vs_factory` 自证 | — | **14**（新） |
| `PASS` | 658 | **665** |
| `DIVERGE` | 83 | **75** |
| 其中 **专有** | 41 | **29** |
| 其中 **上游** | 41 | **46** |
| 新仪器 | — | `audit_vs_factory.py` / `libc_model.py` / `--cache` / `CGM_NO_LIBC_MODEL` |

★ 上游发散从 41 抬到 46 的**原因已查明**：模型让 `mxmlSave*`（5 个）等**首次可观测**。
§17.19 记录上游发散的**根因归属**（版本错配），它是**下一轮的第一优先**。

---

## 17.19 ★★★★★ 上游 46 个发散的根因：**版本错配**（指纹 + 调用签名双向定案）

| 库 | 工厂侧证据 | 我方 | 判定依据 |
|---|---|---|---|
| **mxml** | `_mxml_strdupf` 调 `vsnprintf` + `strdup`；且有独立的 `_mxml_vstrdupf` | **mxml 3.2**（`mxml.h`：`MXML_MAJOR_VERSION 3` / `MINOR 2`），`_mxml_strdupf` 用 **`vasprintf`** | mxml 2.x 的 `_mxml_strdupf` 走 `_mxml_vstrdupf`（`vsnprintf`+`strdup`），3.0 起改用 `vasprintf`（`HAVE_VASPRINTF`）⇒ **工厂 ≤ 2.x，我方 3.2** |
| **libiconv** | `cns11643_*` / `hkscs*` / `big5` 的转换结论不同（`hkscs2001_wctomb` F=`0xfffffffe` O=`0xffffffff`；`cns11643_1_mbtowc` F=ILSEQ O=成功） | **libiconv 1.17**（2022-05-15） | 两侧读的 `*_2uni_*` **码表条目不同** ⇒ 版本不同；工厂是 glibc-2.24/GCC6.2 时代（2016）⇒ 应为 1.14/1.15 级 |
| **libcharset** | `locale_charset` 调 **`nl_langinfo`** | 调 **`getenv` ×3** | libcharset 的 `locale_charset` 实现随版本变过（`nl_langinfo(CODESET)` vs `getenv` 回退链） |
| **zlib/unzip** | `_Z20inflate_blocks_reset` / `_Z25unzlocal_DosDateToTmuDate` 返回值不同 | 我们的 `zip_utils` 版本 | zlib 内部结构与 `DosDateToTm` 的越界处理随版本变 |

**⇒ 根因（一句话）**：这 46 个不是"我们写错了"，而是**我们 vendored 的上游库版本比工厂新**。
**逐函数打补丁在这一类上无解**（改一个函数就会与它自己的库版本冲突）。

**两条可选根解（下一轮二选一，必须显式决策，不许含糊）**：
1. **版本对齐**：`mxml` 降到工厂的 2.x、`libiconv` 降到 1.15/1.16、`zip_utils`/zlib 对齐，
   重编后复测 —— 判据 = "这 46 个里有多少转 PASS"；
2. **契约化**：承认第三方库**内部的**差异不构成缺陷，判据从"逐函数比内部行为"改为
   "**API/ABI 完整性 + 版本确权 + 调用边界的观测量**"，把这 46 个从"发散棘轮"移到独立的
   "**版本差异台账**"（只增不删、附指纹证据）。
   ★ 采用 2 必须同时给出：被移出项的**调用方**（我们的专有代码）是否因此可能行为不同 ——
   即把风险从"库内部"收敛到"库的调用边界"，而不是简单划走。

---

## 17.20 ★★★★★ **公开更正**：门禁总账此前"一个步骤都没检视"，却报 `✓ 全部步骤 success`（§17.11 的结论作废）

### 事实（CI 日志逐字，提交 `12a1dd96`）
...
**`步骤总数：0`** —— 它**一个步骤都没检视**，然后报了全绿。

### 根因

`ci_gate_summary.py` 读的是 `toJSON(steps)`。**`steps` 上下文的键是"步骤 `id`"**，
而 `1to1-verify.yml` 里**没有任何一个步骤带 `id:`** ⇒ `toJSON(steps)` 返回**空对象**
⇒ 解析"成功"、总数 0、失败 0 ⇒ 打印全绿。

★★ 更要命的是它的**输出形态**：`"0 个步骤"` 与 `"全部通过"` 打印出来**一模一样**。
这正是本项目第 3 条纪律（"'没跑'与'跑了但过了'必须可区分"）早该拦住的东西 —— 而它没被拦住，
因为**那条纪律当时只落在"工具内部桶"上，没有落到"检视了多少个步骤"这个元层**。

### 后果（必须承认的两条）

1. **§17.11 当时的结论作废。** 原文写"`362bdf75` 实测：总账打印 `失败 0 步 / ✓ 全部步骤 success`
   ⇒ 35 个门禁**一次跑全并全过**"。**那句"35 个门禁一次跑全"是错的** —— 它检视的是 0 个。
   "全部步骤 success"当时只是**恰好为真**（那轮确实全过），但**不是被验证过的**，是靠运气。
2. **本轮一次真实失败被静默吞掉**：`audit_vs_factory.py --self-test`（正确写法是 `--selftest`）
   报 `error: unrecognized arguments`，因该步骤有 `continue-on-error: true` 而整步被视作成功，
   总账也只报"失败 0 步"。⇒ **新步骤在本轮 CI 里其实没产出报告**，而我当时准备把它写成"已实测"。
   （这条恰恰是靠**逐行读 CI 日志**才发现的，不是靠门禁 —— 门禁当时是瞎的。）

### 修法（三处，缺一不可）

| # | 改动 | 目的 |
|---|---|---|
| 1 | `1to1-verify.yml`：**42 个步骤全部补 `id: sNN`** | 让 `toJSON(steps)` 真的有内容 |
| 2 | `ci_gate_summary.py`：新增 `MIN_STEPS = 30`，`verdict(..., min_steps=MIN_STEPS)`；**检视数 < 下界 ⇒ `parsed_ok=False`**（调用方 fail-closed） | **"解析出 0 个" 必须与 "全部都过" 可区分** |
| 3 | 同文件：把**自证里那条锚点**的期望从 `([], True)`（空 dict = 无失败项）**改成 `([], False)`** | 原锚点**本身就是把假绿固化成了期望** —— 它让"空 ⇒ 绿"变成通过标准 |

另修：`audit_vs_factory.py` 的 `--selftest` 现在**同时接受 `--self-test`**（写错一次就静默失败过一次，
所以让两种写法都成立，比"记住正确写法"更可靠）。

### 纪律（新增第 14 条）

> **14. 元层也要有"没跑"的哨兵。** 逐项判据做对了"桶要列名"，还要**对"检视了多少项"本身设下界**。
> 任何"聚合/总账"类工具，都必须回答三个数：**应检视几个 · 实际检视几个 · 失败几个**，
